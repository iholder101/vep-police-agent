...
def _check_development_phase_risks(
    ...
    for vep_issue_num, board_vep in board_veps.items():
        ...
        # Get implementation PRs (prioritize board data from PR #2)
        impl_prs = board_vep.get("impl_prs", [])

        # Fallback to label-based mappings
        if not impl_prs:
            vep_num_str = str(vep_issue_num)
            impl_prs = vep_to_pr_mappings.get(vep_num_str, [])
...
def _extract_vep_issue_number(text: str) -> int | None:
    """Extract VEP issue number from text.

    Looks for patterns like:
    - Tracking issue: https://github.com/kubevirt/enhancements/issues/123
    - Issue: #123
    - Fixes #123

    Args:
        text: Text to search (PR body, etc.)
    ...
    url_pattern = re.compile(r'github\.com/kubevirt/enhancements/issues/(\d+)')
    match = url_pattern.search(text)
    if match:
        return int(match.group(1))

    # Pattern 2: Issue reference (tracking issue, fixes, closes, etc.)
    ref_pattern = re.compile(r'(?:tracking issue|issue|fixes|closes|related to)[\s:]+#(\d+)', re.IGNORECASE)
